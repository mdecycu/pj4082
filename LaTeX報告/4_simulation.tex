...
\begin{figure}[hbt!]
\center
\includegraphics[width=16cm]{組合圖}
\caption{\Large 組合圖}
\label{組合圖}
\end{figure}
...
\subsection{使用原因}
 本模擬之最終目標是希望可以在虛擬環境中進行3D列印的結果展示，通過虛擬環境中的模擬後，在每次修改零件並更新Gcode後可以直接展示列印的狀況，且因為在虛擬環境中不會有金費的支出，所以可以不斷的重複模擬直到模擬達到最佳的狀態，除此之外CoppeliaSim的虛擬環境更接近真實環境，基於以上原因，所以使用了CoppeliaSim開發。\\
