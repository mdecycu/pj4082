...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[scale=0.74]{正常轉檔流程}
...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[scale=0.74]{pystl轉檔流程}
...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[scale=0.74]{clone pySTL}
...
在這裡我們選擇的檔案是uArm\_binary.stl檔案，並且可以先使用print，使我們可以先取得原本stl檔案的體積與質心數值。 (如:圖5.4)\\
...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[scale=0.74]{ 體積與質心數值顯示}
...
因為stl檔案的單位mm，但是coppeliasim模擬的場景的單位是m，所以我們必須使用scale=0.001，使物體縮小1000倍在使用model.write\_text\_stl來產生檔案。(如圖:圖5.6)\\
...
最後我們把生成好的uArm\_scale\_down\_0.001stl放進coppeliasim場景裡面(圖5.7)我們可以發現uArm手臂的尺寸與方向都是正確的，無須再調整。\\

\begin{figure}[hbt!]
\begin{center}
\includegraphics[scale=0.74]{clone uArm_scale_down_coppeliasim}
...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[scale=0.74]{sample.py程式流程}
...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[scale=0.74]{pySTL.py程式流程}
