...
\end{figure}
 ...
 採用3D列印機中的擠製列印，他的基層製造技術是將材料加熱後施加壓力噴嘴擠壓出來而定型，一層層的堆疊而形成三維的立體形狀。\\
...
\subsection{導入虛擬場景}
將CAD檔轉入虛擬環境時，可能會以不理想的座標系及大小進入到環境為了解決這個問題，採用STL檔轉換為XML檔，因為XML檔可以以說明資料的形式去傳送和攜帶資訊，透過改寫XML檔的方式可以控制轉入場景的物件的各項數值。\\
\begin{itemize}
%=----------Sigmoid      Function----------=%
...
在3D情況下，計算的基本單元為四面體，將原點與三角形的各個頂點連接形成一個四面體。三角形ACB具有法線NACB，由於原點 O 位於 NACB 的對面，因此
這個四面體的值是正的，體積也可以由內積 ( OA \cdot NACB ) 判斷。四面體體積為:\\
$$ V'_t = \frac{1}{6}(-x_i3 y_2i z_1i + x_2i y_3i z_1i + x_3i y_1i z_2i - x_1i y_3i z_2i - x_2i y_1i z_3i + x_1i y_2i z_3i) $$
...
%=----------Relu Function----------=%
\item 矩和傅立葉轉換：\\
計算的特徵可以寫為帶有作標總和的基礎特徵形狀，並且可以以顯式倒出基本形狀。雖然這是一個很大的約束，但是在內部空間上具有整合的形式都可以使用。\\
 p, q, r 為力矩的階數，中心力矩可以從下列求解。\\
\end{itemize}
$$M_pqr = \iiint x^{p} y^{q} z^{r} \rho (x,y,z) \,dx\,dy\,dz$$
其中 ρ(x, y,z) 是指示函數：\\
\begin{equation}
\label{eq6}
\rho (x,y,z) = \ieft\{
