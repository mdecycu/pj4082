...
\fontsize{14pt}{20pt}\sectionef\hspace{12pt}\quad 此專題是使用虛擬建構的3D列印機，將其導入CoppeliaSim模擬環境並給予對應設置，並使用切片軟體轉出Gcode檔後，運用GCodeInterpreter導入Gcode檔與CoppeliaSim進行對接，操控CoppeliaSim場景中的三軸3D列印機進行模擬列印，可藉此展示與模擬實際列印的情況。\\[12pt]

\end{flushleft}
\begin{center}
\fontsize{14pt}{20pt}\selectfont 關鍵字: \sectionef CoppeliaSim、3D printer、FDM
\end{center}
\newpage
%=--------------------Abstract----------------------=%
\renewcommand{\baselinestretch}{1.5} %設定行距
